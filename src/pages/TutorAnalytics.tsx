        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="flex-1 bg-gradient-to-r from-primary to-indigo-600 text-white rounded-xl py-2.5 text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Verify PIN
            </button>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center p-3 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-2">Avg Score</p>
            </div>
            <div className="text-center p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center justify-center">
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-2">Grade</p>
            </div>
            <div className="text-center p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center justify-center">
              <span className="text-3xl font-black text-slate-800">{student.completed}</span>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-2">Modules</p>
            </div>
          </div>

          {/* Last Active */}
          <div className="flex items-center gap-2 text-xs text-slate-500 font-medium bg-slate-50 rounded-xl px-4 py-2.5 border border-slate-100">
              ...
              {editingScore && isTutor ? (
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <div className="flex justify-between text-[10px] text-slate-400 font-bold mt-1">
                      <span>0%</span>
                      <span className="text-primary font-black text-sm">{tempScore}%</span>
                      <span>100%</span>
                    </div>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button onClick={() => setEditingScore(false)} className="border border-slate-200 text-slate-600 text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer">
                      Cancel
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-black text-slate-800">{student.avgScore}</span>
                    <span className="text-slate-400 font-bold">/ 100</span>
                  </div>
                  {!isTutor && (
                    <p className="text-[10px] text-slate-400 font-medium italic">Unlock Tutor Mode to edit grades</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="space-y-2">
            ...
            {isTutor && (
              <button
                ...
                className="w-full py-3 rounded-xl text-sm font-bold border border-red-200/60 bg-red-500/10 hover:bg-red-500/20 text-red-600 flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
...
export default function TutorAnalytics() {
  ...
  const filteredStudents = useMemo(() => {
    ...
    list.sort((a, b) => {
      const va = sortBy === 'name' ? a.name : sortBy === 'score' ? a.avgScore : a.completed;
      const vb = sortBy === 'name' ? b.name : sortBy === 'score' ? b.avgScore : b.completed;
  ...
  const handleUnlockTutor = () => {
    setIsTutorMode(true);
    setShowPinModal(false);
    showToast('Tutor Mode activated. You can now edit student records.', 'success');
  };
  const handleLockTutor = () => {
    setIsTutorMode(false);
    showToast('Tutor Mode locked.', 'info');
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          {isTutorMode ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-bold hover:bg-slate-50 hover:border-primary/30 transition-colors"
            >
          ...
          {isTutorMode && (
            <Button onClick={() => setIsNewStudentOpen(true)} className="flex items-center gap-1.5 text-xs">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Enrolled" value={totalStudents} sub="Active learners" icon={Users} color="bg-purple-50 text-primary" delta={{ val: '+2 this week', up: true }} />
        <StatCard label="Cohort Average" value={`${averageCohortScore}%`} sub="Mean score" icon={TrendingUp} color="bg-emerald-50 text-emerald-600" delta={{ val: '+3% vs last week', up: true }} />
        <StatCard label="At Risk" value={studentsAtRisk} sub="Need intervention" icon={AlertTriangle} color="bg-red-50 text-red-500" />
        <StatCard label="Completions" value={totalCompletedLessons} sub="Total modules done" icon={BookOpen} color="bg-blue-50 text-blue-500" delta={{ val: '+12 this week', up: true }} />
      </div>

      {/* ── Chart + Leaderboard ─────────────────────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 bg-white border-slate-100 p-6">
          <h3 className="text-sm font-extrabold text-slate-800 mb-1">Grade Distribution Matrix</h3>
          <p className="text-xs text-slate-400 font-medium mb-4">Real-time score breakdown across cohort</p>
        ...
        <Card className="bg-white border-slate-100 p-6">
          ...
          <p className="text-xs text-slate-400 font-medium mb-4">Highest scoring students this period</p>
          <div className="space-y-3">
            {topStudents.map((s, i) => (
              <div key={s.id} className="flex items-center gap-3">
                ...
                <img src={s.avatar} alt={s.name} className="w-8 h-8 rounded-full border border-slate-100 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-slate-800 truncate">{s.name}</p>
                  <div className="w-full bg-slate-100 h-1.5 rounded-full mt-1 overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-primary to-cyan-400 h-full rounded-full"
                      style={{ width: `${s.avgScore}%`, transition: 'width 0.6s ease' }}
                    />
                  </div>
                </div>
                <span className="text-xs font-black text-slate-700 shrink-0">{s.avgScore}%</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card className="bg-white border-slate-100 p-0 overflow-hidden rounded-3xl">
        <div className="px-6 py-4 bg-slate-50/80 border-b border-slate-100">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="text-left">
              <h3 className="text-sm font-extrabold text-slate-800">Enrollment Roster</h3>
              <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                {isTutorMode ? '🔓 Tutor Mode — click rows to edit' : '🔒 View-only — unlock Tutor Mode to edit'}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-2 text-xs border border-slate-200 bg-white rounded-xl focus:outline-none focus:border-primary transition-colors font-medium text-slate-800"
                />
              </div>
              <select
                value={filterRisk}
                onChange={e => setFilterRisk(e.target.value as any)}
                className="text-xs border border-slate-200 bg-white rounded-xl px-3 py-2 focus:outline-none focus:border-primary font-bold text-slate-600"
              >
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredStudents.length === 0 ? (
                ...
                <tr
                  key={s.id}
                  className={`font-medium transition-colors border-b border-slate-100 ${
                    isTutorMode ? 'hover:bg-purple-50/30 cursor-pointer' : 'hover:bg-slate-50/40'
                  }`}
                ...
                >
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <div className="relative shrink-0">
                        <img src={s.avatar} alt={s.name} className="w-9 h-9 rounded-full border border-slate-100 object-cover" />
                        <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${s.risk ? 'bg-red-500' : 'bg-emerald-500'}`} />
                      </div>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-slate-800 text-sm">{s.avgScore}%</span>
                    </div>
                  </td>
                  <td className="py-4 px-6 text-slate-500 text-sm">{s.completed} modules</td>
                  <td className="py-4 px-6 text-slate-400 text-sm">{s.lastActive}</td>
                  <td className="py-4 px-6">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase border ${
                      s.risk ? 'bg-red-50 border-red-100 text-red-600' : 'bg-emerald-50 border-emerald-100 text-emerald-600'
                    }`}>
                  ...
                  <td className="py-4 px-6 text-right">
                    <button
                      ...
                      className="text-primary font-bold text-xs hover:underline hover:text-indigo-700 flex items-center gap-1 ml-auto"
                    >
        ...
        <div className="px-6 py-3 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
          ...
          <button
            ...
            className="text-[11px] text-slate-400 font-bold hover:text-primary transition-colors flex items-center gap-1 cursor-pointer"
          >
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden border border-slate-100">
            <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              ...
              <button
                ...
                className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-200/50 rounded-xl cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <form onSubmit={handleCreateStudent} className="p-6 space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none p-3 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
                <input
                  type="checkbox"
                  checked={newRisk}
                  onChange={e => setNewRisk(e.target.checked)}
                  className="h-4 w-4 accent-red-500 rounded cursor-pointer"
                />
                <div>
              ...
              <div className="pt-2 border-t border-slate-100 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setIsNewStudentOpen(false)}
                  className="px-4 py-2 text-xs font-bold border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 text-xs font-bold bg-gradient-to-r from-primary to-indigo-600 text-white rounded-xl shadow-sm hover:from-purple-600 hover:to-indigo-700 transition-all"
                >
                  Enroll Student
                </button>
              </div>
